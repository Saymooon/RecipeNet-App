...
from io import BytesIO # ⭐️ [추가] 엑셀 다운로드를 위해 import
...
# ⭐️ [수정됨] 엑셀 다운로드를 위한 헬퍼 함수 (헤더 정보 포함)
def to_excel_with_header(recipe_df, color_name, date_str):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        for idx, col in enumerate(recipe_df): # recipe_df의 컬럼 기준
            ...
            # 헤더 정보 컬럼 너비도 고려 (단순히 최대 20으로 설정)
            worksheet.column_dimensions[chr(66 + idx)].width = max(max_len, 20) # B열부터 시작 가정
        # 헤더 정보 컬럼 너비
...
# ==========================================================
# ⭐️ [수정됨] 이 함수 전체가 수정되었습니다. (엑셀 다운로드 로직 변경)
def run_inference(model, cfg, surrogate, spectrum, lab, color_name, name_encoder):
    ...
    # ⭐️ [엑셀 다운로드 수정] 헤더 정보 포함 함수 호출
    excel_data = to_excel_with_header(recipe_df_for_download_and_display, color_name, current_date)
    st.download_button(
        # ⭐️ 버튼 레이블 수정
